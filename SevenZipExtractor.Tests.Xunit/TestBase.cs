...
namespace SevenZipExtractor.Tests.Xunit
{
    public abstract class TestBase
    {
        protected IList<TestFileEntry> TestEntriesWithFolder = new List<TestFileEntry>()
        {
                ...
                new TestFileEntry { Name = "testFolderimage3.jpg", IsFolder = false, MD5 = "24ffd227340432596fe61ef6300098ad"},
                new TestFileEntry { Name = "画像4.jpg", IsFolder = false, MD5 = "e6efee68e730ec3dba7adbcc6b221281"},
                new TestFileEntry { Name = "zdjęcie5.jpg", IsFolder = false, MD5 = "63538f657330b1f348885f3d0a9bc88f"},
                new TestFileEntry { Name = "🜀🜁🜂.png", IsFolder = false, MD5 = "9d8ea7f8559a02c72b3e5208a03cde74"},
        };
            ...
            using (ArchiveFile archiveFile = new ArchiveFile(memoryStream, sevenZipFormat))
            {
                foreach (TestFileEntry testEntry in expected)
                {
                    ...
                    using (MemoryStream entryMemoryStream = new MemoryStream())
                    {
                        ...
                        {
                            Assert.Equal(testEntry.MD5, entryMemoryStream.ToArray().MD5String());
                        }

                        if (testEntry.CRC32 != null)
                        {
                            Assert.Equal(testEntry.CRC32, entryMemoryStream.ToArray().CRC32String());
                        }
                    }
                }
            }
