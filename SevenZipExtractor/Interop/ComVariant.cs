...
//Changes were made to this file to match 7zip native library.
using System;
...
namespace SevenZipExtractor.Interop
{
    ...
    public struct ComVariant7Zip : IDisposable
    {
        [DllImport("ole32.dll")]
        private static unsafe extern int PropVariantClear(ComVariant7Zip* pvar);
        ...
        static unsafe ComVariant7Zip()
        {
            ...
            int variantSize = sizeof(ComVariant7Zip);
            if (IntPtr.Size == 4)
            {
        ...
        private struct TypeUnion
        {
            ...
            //TODO: Therefore for big endian machine those two must be flipped.
            public ushort _vt;
        ...
        /// <summary>
        /// Release resources owned by this <see cref="ComVariant7Zip"/> instance.
        /// </summary>
        public unsafe void Dispose()
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                fixed (void* pThis = &this)
                {
                    ...
                    PropVariantClear((ComVariant7Zip*)pThis);
                }
            }
            else
            {
                ...
                {
                    SevenZipHandle.thisHandle!.BSTR.Free(_typeUnion._unionTypes._bstr);
                }
                ...
                else if (VarType.HasFlag(VarEnum.VT_VECTOR))
                {
                    switch (VarType & ~VarEnum.VT_VECTOR)
                    {
                        case VarEnum.VT_BSTR:
                            ...
                            {
                                SevenZipHandle.thisHandle!.BSTR.Free(str);
                            }
                            break;
                        ...
                        case VarEnum.VT_VARIANT:
                            foreach (var variant in GetRawDataRef<Vector<ComVariant7Zip>>().AsSpan())
                            {
                                variant.Dispose();
                            }
                            break;
...
#pragma warning disable CS0618 // We support the obsolete CurrencyWrapper type
        /// <summary>
        /// Create an <see cref="ComVariant7Zip"/> instance from the specified value.
        /// </summary>
        /// <typeparam name="T">The type of the specified value.</typeparam>
        /// <param name="value">The value to wrap in an <see cref="ComVariant7Zip"/>.</param>
        /// <returns>An <see cref="ComVariant7Zip"/> that contains the provided value.</returns>
        ...
        public static ComVariant7Zip Create<T>([DisallowNull] T value)
        {
            ...
            ComVariant7Zip variant = default;
            ...
            else if (typeof(T) == typeof(BStrWrapper))
            {
                ...
                {
                    variant._typeUnion._unionTypes._bstr = BStrMarshaller7Zip.ManagedToNative(((BStrWrapper)(object)value).WrappedObject!);
                }
            }
            else if (typeof(T) == typeof(string))
            {
                ...
                // to do so.
                variant.VarType = VarEnum.VT_BSTR;
                variant._typeUnion._unionTypes._bstr = BStrMarshaller7Zip.ManagedToNative((string)(object)value);
            }
...
#pragma warning restore CS0618
        ...
        /// <summary>
        /// Create a <see cref="ComVariant7Zip"/> with the given type and provided value.
        /// </summary>
        ...
        public static unsafe ComVariant7Zip CreateRaw<T>(VarEnum vt, T rawValue)
            where T : unmanaged
        {
            ...
            value.GetRawDataRef<T>() = (vt, sizeof(T)) switch
            {
                ...
                _ => throw new ArgumentException(string.Format("Size of {0} should be the same size as the value specified by {1}", nameof(T), nameof(vt)))
            };
        ...
        /// <summary>
        /// A <see cref="ComVariant7Zip"/> instance that represents a null value with <see cref="VarEnum.VT_NULL"/> type.
        /// </summary>
        public static ComVariant7Zip Null { get; } = new() { VarType = VarEnum.VT_NULL };

        private readonly void ThrowIfNotVarType(params VarEnum[] requiredType)
        {
            if (Array.IndexOf(requiredType, VarType) < 0)
            {
                throw new InvalidOperationException(string.Format("Variant type '{0}' is not one of the supported variant types for this operation: [{1}]", VarType, string.Join(", ", requiredType)));
            }
        }

#pragma warning disable CS0618 // Type or member is obsolete
        /// <summary>
        /// Create a managed value based on the value in the <see cref="ComVariant7Zip"/> instance.
        /// </summary>
        ...
        /// <exception cref="ArgumentException">When <typeparamref name="T"/> does not match the <see cref="VarType"/> of the <see cref="ComVariant7Zip"/>.</exception>
        public readonly T? As<T>()
        {
            ...
            else if (typeof(T) == typeof(decimal))
            {
                ...
                ComVariant7Zip variantWithoutVarType = this;
            ...
            throw new ArgumentException("The method returned a COM Variant type that is not Interop compatible.", nameof(T));
        }
#pragma warning restore CS0618 // Type or member is obsolete

        /// <summary>
        /// The type of the data stored in this <see cref="ComVariant7Zip"/>.
        /// </summary>
        ...
        /// <summary>
        /// Get a reference to the storage location within this <see cref="ComVariant7Zip"/> instance.
        /// </summary>
        /// <typeparam name="T">The type of reference to return.</typeparam>
        /// <returns>A reference to the storage location within this <see cref="ComVariant7Zip"/>.</returns>
        /// <exception cref="ArgumentException"><typeparamref name="T"/> is <see cref="decimal"/> or larger than the storage space in an <see cref="ComVariant7Zip"/>.</exception>
        [UnscopedRef]
